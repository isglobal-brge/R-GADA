		...
		if (not @fhout) {					#reading the first line of the input stream
			...
			for my $i (1 .. $num_fh) {
				...
				if ($name_by_header) {			#output file name is based on the corresponding column in header line
					...
					open ($temp_fh, ">$output.split$file_index") or die "Error: cannot open file handle for writting to $output.split$file_index: $!";
				}
				push @fhout, $temp_fh;
			}
			if ($name_by_header) {
				print STDERR "NOTE: individual files will be written to ${\(scalar @fhout)} files with name starting as $output and ending as the sample name (as specified in first line of input file)\n";
			} elsif ($start_split) {
				print STDERR "NOTE: individual files will be written to files $output.split$start_split to $output.split", $start_split+$num_fh-1, "\n";
			} else {
				print STDERR "NOTE: individual files will be written to files $output.split1 to $output.split$num_fh\n";
			}
		}
